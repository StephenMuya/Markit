...
def push_deals_to_notion():
    ...
    for deal in deals:
        ...
        existing = notion.databases.query(
            ...
            filter={"property": "Article ID", "rich_text": {"equals": deal["article_id"]}}
        )
        if existing["results"]:
        ...
        # Link Firms
        equity_partner_id = get_or_create_firm(deal.get("equity_partner"), "Equity Partner")
        developer_id = get_or_create_firm(deal.get("developer"), "Developer")
        
        props = {
            "Deal Headline": {"title": [{"text": {"content": f"{deal.get('equity_partner')} + {deal.get('developer')} - {deal.get('structure')}"}}]},
            "Transaction Summary": {"rich_text": [{"text": {"content": deal.get("summary", "")}}]},
            ...
            "Article ID": {"rich_text": [{"text": {"content": deal.get("article_id") if deal.get("article_id") else ""}}]}
        }
